...
var encodedData = require('soop').load('darkcore/util/EncodedData', {
  base58: base58
});
var versionedData = require('soop').load('darkcore/util/VersionedData', {
  parent: encodedData
});

var Address = require('soop').load('darkcore/lib/Address', {
...
TransactionDb.prototype.getPoolInfo = function(txid, cb) {
  ...
  Rpc.getTxInfo(txid, function(err, txInfo) {
    ...
    if (txInfo && txInfo.isCoinBase) {
      ...
      if (!ret  && txInfo.vout) {
        var addrs='';
        for (var ii in txInfo.vout) {
          var o = txInfo.vout[ii];
          if (o.scriptPubKey && o.scriptPubKey.addresses &&
            o.scriptPubKey.addresses[0] && !o.scriptPubKey.addresses[1] // TODO : not supported=> standard multisig
          ) {
            addrs += o.scriptPubKey.addresses[0];
          }
        }

        ret = self.poolMatch.strmatch(addrs);
      }
      if (!ret) {
        var cbstring = new Buffer(txInfo.vin[0].coinbase, 'hex');
        ret = {poolName: new String(cbstring).replace(/[^\x20-\x7E]+/g, '') };
      }
    }
